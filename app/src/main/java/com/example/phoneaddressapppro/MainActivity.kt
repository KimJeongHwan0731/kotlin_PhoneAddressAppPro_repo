...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (status == PackageManager.PERMISSION_GRANTED) {
            ...
            Toast.makeText(this, "주소록앱 정보 가져오기 요청 안되어있음", Toast.LENGTH_SHORT).show()
        ...
        // 2. 주소록 정보를 요청했을때 주소록앱에서 선택된 주소Uri 보내줄때 받을 콜백함수
        ...
        // 3. 이벤트 처리 (주소록앱 Activity 인텐트를 통해서 부른다. -> 주소록 클릭하면 클릭된 주소록 Uri를 돌려준다.
        binding.button.setOnClickListener {
            // 주소록에서 전화번호앱 리스트에서 전화번호를 가지고 있는 사람만 리사이클러뷰로 보여주라 요청
            val intent =
